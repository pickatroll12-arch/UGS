...
(function (root, factory) {
  ...
  function create() {
    ...
    const runs = new Map();      // roomId -> { segs, i, t, loop }
    let running = false;

    function findRoom(level, id) { return level.rooms.find(r => r.id === id) || null; }

    function fire(room, event) {
      const segs = buildSegments(event, poseOfRoom(room));
      if (segs.length) runs.set(room.id, { segs, i: 0, t: 0, loop: !!event.loop });
    }
